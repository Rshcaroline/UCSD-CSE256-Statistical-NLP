...
@LastEditTime: 2019-10-12 15:22:29
...
if __name__ == "__main__":
    ...
    # Learn the models for each of the gutenberg, and evaluate it
    # unigram
    # model = learn_unigram(data[trainIndex])
    # trigram
    model = learn_trigram(datas[trainIndex], delta=1/2**(15), smoothing=True, verbose=True)

    # compute the perplexity of reuters
    perp_test = model.perplexity(datas[testIndex].test)
    print("perplexity of ", dnames[trainIndex], " on ", dnames[testIndex], " is: ", perp_test)
    ...
    for r in ratio:
        print("ratio: ", r)
        retrainModel = model
        data = datas[testIndex]
        retrainData = data.train[:int(r*len(data.train))]
        retrainModel.fit_corpus(retrainData)
        print("vocab:", len(retrainModel.vocab()))
        # evaluate on train, test, and dev
        print("train:", retrainModel.perplexity(retrainData))
        print("dev  :", retrainModel.perplexity(data.dev))
        print("test :", retrainModel.perplexity(data.test))
        perp_trains.append(retrainModel.perplexity(retrainData))
        perp_tests.append(retrainModel.perplexity(data.test))

    plt.plot(ratio, perp_trains, label='Train Perplexity')
    plt.plot(ratio, perp_tests, label='Test Perplexity')
